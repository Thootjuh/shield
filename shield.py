...
from PrismEncoder import encodeCartPole, encodeCrashingMountainCar, encodeMaze, encodeLunarLander, encodeFrozenLake, encodeMovingObstacles
import pycarl
import subprocess
...
class ShieldMovingObstacles(Shield):
    # Calculate the shield for the Random MDPs environment
    def __init__(self, transition_matrix, traps, goal, intervals, init):
        ...
        self.prism_text = encodeMovingObstacles(transition_matrix, intervals, traps, goal)
        super().__init__(transition_matrix, traps, goal, intervals)
...
class ShieldCartpole(Shield):
    ...
    def get_safe_actions_from_shield(self, state, threshold=0.0, buffer = 0.025):
        """
        calculate the actions allowed by the shield for a given state
        Args:
            state (int): the state for which we want compute the safe actions
...
class ShieldLunarLander(Shield):
    # Calculate the shield for the Random MDPs environment
    def __init__(self, transition_matrix, traps, goal, intervals, init):
        """
        args:
