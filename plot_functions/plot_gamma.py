...
def extract_data(data):
    ...
    relevant_data_new = data[['method', 'length_trajectory', 'discounted_method_perf', 'run_time', 'nb_states']]
...
def plot_data(data, filename):
    ...
    data = grouped_data.discounted_method_perf.mean().reset_index()
    # Plot average performance against length_trajectory for each method
    plt.figure(figsize=(12, 8))
    for method in data['method'].unique():
        method_data = data[data['method'] == method]
        plt.plot(method_data['length_trajectory'], method_data['discounted_method_perf'], label=method)
...
plot_data(data, 'Results_gamma.png')
