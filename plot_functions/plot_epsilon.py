...
def read_data_from_directory(directory_path):
    ...
    for file in csv_files:
        ...
        data = data.rename(columns={'baseline_target_perf_ratio': 'epsilon_baseline'})
...
def extract_data(data):
    ...
    relevant_data_new = data[['method', 'epsilon_baseline', 'method_perf', 'run_time']]

    # Group by method and calculate the average performance for each epsilon_baseline
...
def plot_data_interval(data, filename, method_name):
    grouped_data = data.groupby(['method', 'epsilon_baseline'])
    ...
    plt.title(f'Average Performance vs. Data Quality for {method_name}')
...
def plot_data(data, filename, method_name):
    ...
    # Plot average performance against epsilon_baseline for each method
    plt.figure(figsize=(12, 8))
    for method in data['method'].unique():
        method_data = data[data['method'] == method]
        plt.plot(method_data['epsilon_baseline'], method_data['method_perf'], label=method)
...
def calculate_cvar(data, alpha=0.01):
    ...
    for method in data['method'].unique():
            method_data = data[data['method'] == method]
            for traj in method_data['epsilon_baseline'].unique():
                traj_data = method_data[method_data['epsilon_baseline'] == traj]['method_perf']
                if len(traj_data) > 0:
                    ...
                    cvar_results.append({'method': method, 'epsilon_baseline': traj, 'cvar': cvar})
...
def plot_cvar(cvar_data, filename, method_name):
   # Plot CVaR against trajectory for each method
    ...
    for method in cvar_data['method'].unique():
        ...
        plt.plot(method_data['epsilon_baseline'], method_data['cvar'], label=method)

    # Set plot labels and legend
    plt.xlabel('Epsilon')
    plt.ylabel('1%-CVaR (Conditional Value at Risk)')
    plt.title(f'1%-CVaR vs. Data quality for {method_name}')
    plt.legend(title='Method')
    plt.grid(True)
    plt.savefig(filename)
    plt.show()
...
def plot_all_methods(data, filename):
    ...
    for method in sorted_methods:
        ...
        plt.plot(method_data['epsilon_baseline'], method_data['method_perf'],
                 label=method, linestyle=linestyle, color=color)
    
    plt.xlabel('Epsilon')
    plt.ylabel('Average Method Performance')
    plt.title('Average Performance vs. Data quality for all methods')
