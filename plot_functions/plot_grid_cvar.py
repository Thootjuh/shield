...
def plot_all_methods(ax, data, title, color):
    ...
    data = grouped_data.cvar.mean().reset_index()
    ...
    for method in sorted_methods:
        ...
        line, = ax.plot(method_data['length_trajectory'], method_data['cvar'],
                 label=method, linestyle=linestyle, color=color)
        legend_handles.append(line)
        legend_labels.append(method)
    
    ax.set_xlabel('Length Trajectory', fontsize="15")
    ax.set_ylabel('1%-CVar Performance', fontsize="15")
...
def plot_multiple_directories(parent_directory, output_filename):
    ...
    for directory in directories:
        ...
        data_cvar = calculate_cvar(data)
        data_list.append(data_cvar)        
...
plot_multiple_directories(parent_directory, "combined_results_cvarBIG.png")
