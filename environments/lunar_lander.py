...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def lander_body_overlaps_moon(self, position, angle):
        """
        Pure geometric check: does the lander body at (position, angle)
        intersect the moon terrain?

        The environment state is restored exactly afterward.
        """
        ...
        try:
            # --- Move body correctly using transform property ---
            self.lander.transform = (position, angle)

            lander_fixture = self.lander.fixtures[0]

            # --- Collision test ---
            for moon_fixture in self.moon.fixtures:
                if Box2D.b2TestOverlap(
                    lander_fixture.shape,
                    0,
                    moon_fixture.shape,
                    0,
                    self.lander.transform,
                    self.moon.transform,
                ):
                    return True
    ...
    def set_state(self, state):
        ...
        Parameters
        ----------
        state : list or array-like of length 8
            [x, y, vx, vy, angle, angular_velocity, leg_left_contact, leg_right_contact]
            Same format as the observation returned by step().
        """
        assert self.lander is not None, "Environment must be reset before calling set_state()"
        assert len(state) == 8, "State must contain exactly 8 values"

        state = np.asarray(state, dtype=np.float32)

        # ----- Reconstruct lander position -----
        # In step():
        # state[0] = (pos.x - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2)
        # state[1] = (pos.y - (helipad_y + LEG_DOWN/SCALE)) / (VIEWPORT_H/SCALE/2)

        W = VIEWPORT_W / SCALE
        H = VIEWPORT_H / SCALE

        pos_x = state[0] * (W / 2) + (W / 2)
        pos_y = state[1] * (H / 2) + (self.helipad_y + LEG_DOWN / SCALE)
        ...
        # state[5] = 20 * angularVelocity / FPS
        angular_velocity = state[5] * FPS / 20.0

        # ----- Apply to Box2D bodies -----
        self.lander.position = (pos_x, pos_y)
        self.lander.linearVelocity = (vel_x, vel_y)
        self.lander.angle = angle
        self.lander.angularVelocity = angular_velocity

        # Wake body to ensure physics updates continue
        self.lander.awake = True

        # ----- Legs ground contact flags -----
        self.legs[0].ground_contact = bool(state[6])
        self.legs[1].ground_contact = bool(state[7])

        # Reset episode flags that depend on trajectory history
