...
class CustomTaxiEnv(TaxiEnv):
    ...
    def _build_rainy_transitions(self, row, col, pass_idx, dest_idx, action):
            ...
            if action <= 3:
                ...
                self.P[state][action].append((0.1, left_state, left_pos[2], terminated))
                self.P[state][action].append((0.1, right_state, right_pos[2], terminated))
            else:
