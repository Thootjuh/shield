...
class ContinuousFrozenLakeEnv(Env):
    ...
    def get_tile_from_continuous_state(self, state):
        """
        Convert a continuous state to the tile character.
        """
        row = int(np.floor(state[0]))
        col = int(np.floor(state[1]))

        row = np.clip(row, 0, self.nrow - 1)
        col = np.clip(col, 0, self.ncol - 1)

        return self.desc[row, col].decode("utf-8")
    
    def _state_to_continuous(self, s):

        row = s // self.ncol
        col = s % self.ncol

        return np.array([row, col]) + self.gen_offset()
    ...
    def step(self, a):
        ...
        return obs, r, terminated, False, {"prob": p}
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        obs = self._state_to_continuous(self.s)
